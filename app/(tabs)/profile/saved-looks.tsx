...
export default function Screen() {
  const items = useSavedStore(s => s.looks);
  ...
  if (items.length === 0) {
    return (
      <SafeAreaView style={s.root} edges={['top']}>
        ...
        <View style={{ flex: 1 }}>
          <EmptyView icon="heart-outline" title="No saved looks yet" body="Recreate a look and save it here for later." actionLabel="Recreate a Look" onAction={() => router.push('/(tabs)/recreate' as any)} />
        </View>
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView style={s.root} edges={['top']}>
      <ScreenHeader title={`Saved Looks · ${items.length}`} />
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scroll}>
        {items.map((it, i) => (
          <Reveal key={it.id} delay={i * 50}>
            <View style={s.card}>
              {it.img ? <AppImage uri={it.img} style={s.thumb} /> : <View style={[s.thumb, s.thumbFallback]}><Ionicons name="heart-outline" size={22} color={Colors.brand.plum} /></View>}
              <View style={{ flex: 1 }}>
                <Text style={s.title} numberOfLines={2}>{it.title}</Text>
                {it.subtitle ? <Text style={s.sub}>{it.subtitle}</Text> : null}
              </View>
              <PressableScale style={s.removeBtn} onPress={() => { toggle('looks', it); toast.info('Removed from saved'); }}>
